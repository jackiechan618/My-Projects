(function(){
    ...
    app.controller("shorUrlCtrl",["$scope", "$http", function($scope, $http){  
        $http.get('/index2/shortUrl').success(function (data){
            if(data.length > 1) {
                $scope.shortUrl = data;
            }
        });
        ...
        $scope.backtohome = function(){
            $http.post('/index1', $scope.longUrl).success(function(){
...
//            }
        ...
        $scope.courseListSubmit = function(){ 
            $scope.sendArray.push($scope.longUrl);
            $http.post('/formPage2', $scope.sendArray).success(function(){
                console.log('Data update');
            })
            .error(function(data) {
                console.log('Data update failure');
